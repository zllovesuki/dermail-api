...
var getSelf = function() {
    return new Promise(function(resolve, reject) {
        superagent.get('http://' + KUBERNETES_SERVICE_HOST + ':' + KUBERNETES_SERVICE_PORT + '/api/v1/namespaces/' + POD_NAMESPACE + '/pods/' + POD_NAME)
...
var getEndpoints = function() {
    return new Promise(function(resolve, reject) {
        superagent.get('http://' + KUBERNETES_SERVICE_HOST + ':' + KUBERNETES_SERVICE_PORT + '/api/v1/namespaces/' + RETHINK_NAMESPACE + '/endpoints/' + RETHINK_CLUSTER)
        .set('Authorization', 'Bearer ' + token)
        .ca(ca)
        .end(function(err, res) {
            if (err) {
