...
var self = module.exports = {
	...
	getOrCreateAddress: Promise.method(function(r, one, accountId) {
		...
		.table('addresses', {readMode: 'majority'})
		.getAll([account, domain, accountId], {index: 'accountDomainAccountId'})
		.pluck('addressId', 'internalOwner', 'aliasOf', 'hold')
		.slice(0, 1)
		.run(r.conn)
		.then(function(cursor) {
			return cursor.toArray();
		...
		.then(function(result) {
			if (result.length === 0) {
				...
				// User sets to hold this address, do not update displayName
				if (address.hold === true) return addressId;
	...
	getArrayOfAddress: Promise.method(function(r, accountId, addresses) {
		if (typeof addresses !== 'object') addresses = [];
		var arrayOfAddress = [];
		return Promise.map(addresses, function(one) {
			if (!one) return;
			return self
			.getOrCreateAddress(r, one, accountId)
			.then(function(addressId) {
				arrayOfAddress.push(addressId);
				return;
			})
		}, { concurrency: 3 })
		.then(function() {
