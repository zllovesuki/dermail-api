...
var self = module.exports = {
		...
		var transform = function(p) {
			...
			return p.map(function(doc) {
				return doc.merge(function() {
					return {
						post: {
							folder: r.db('dermail').table('folders', {readMode: 'majority'}).get(doc('post')('folder'))
						}
					}
				})
			})
	...
	*/
		...
		var evaluation = {
				...
				for (var k = 0, len = arrayOfFrom.length; k < len; k++) {
					...
					for (var i = 0, rlen = results.length; i < rlen; i++) {
						...
						for (var j = 0, flen = from.length; j < flen; j++) {
							var eval = from[j].address ? from[j].address.toLowerCase() : '';
				...
				for (var k = 0, len = arrayOfTo.length; k < len; k++) {
					...
					for (var i = 0, rlen = results.length; i < rlen; i++) {
						...
						for (var j = 0, flen = to.length; j < flen; j++) {
							var eval = to[j].address ? to[j].address.toLowerCase() : '';
							var regex = globToRegExp(email);
							if (regex.test(eval)) {
								filtered.push(results[i]);
								modified = true;
			...
			subject: function(results, subject) {
				...
				for (var i = 0, rlen = results.length; i < rlen; i++) {
					...
					subjectAC.search(results[i].subject ? results[i].subject.toLowerCase() : '', function(found_word) {
			...
			contain: function(results, contain) {
				...
				for (var i = 0, rlen = results.length; i < rlen; i++) {
					...
					containAC.search(results[i].text ? results[i].text.toLowerCase() : '', function(found_word) {
			...
			exclude: function(results, exclude) {
				for (var i = 0, rlen = results.length; i < rlen; i++) {
					if (!self.containsAll(results[i].text ? results[i].text.toLowerCase() : '', exclude)) {
						filtered.push(results[i]);
						modified = true;
					}
				}
	...
	isFalseReply: function(message) {
		...
		var firstThree = message.subject.substring(0, 3).toLowerCase();
		var firstFour = message.subject.substring(0, 4).toLowerCase();
		if ( (firstThree === 're:' || firstThree === 'fw:' || firstFour === 'fwd:') && typeof message.inReplyTo === 'undefined') {
			return true;
		}
		return false;
	},
	isSPFAndDKIMValid: function(message) {
		var spf = message.spf || 'none';
		if (self.acceptableSPF.indexOf(spf) === -1) return false;
		var dkim = message.dkim || [];
		for (var i = 0, len = dkim.length; i < len; i++) {
			if (self.acceptableDKIM.indexOf(dkim[i].result) === -1) return false;
		}
		return true;
	}
}
