...
var self = module.exports = {
	...
	getInternalFolder: Promise.method(function(r, accountId, name) {
		...
		.getAll([name, accountId], {index: 'inboxAccountId'}) // Check Account-Folder Mapping
		.pluck('folderId')
		.slice(0, 1)
		.run(r.conn)
    ...
    getFoldersFromAccounts: Promise.method(function(r, accountIds) {
        return r
        .table('folders', { readMode: 'majority' })
        .getAll(r.args(accountIds), { index: 'accountId' })
        .run(r.conn)
		.then(function(cursor) {
            return cursor.toArray()
        })
